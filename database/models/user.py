...
class User(Base):
    ...
    @classmethod
    async def get_or_create(cls, id: int, name: str, username: str | None = None, lang: str | None = None, referer: str | None = None):
        user = await cls.get(id)
        user = await cls.update(user.id, name=name, username=username) if user else \
            await cls.create(_id=id, name=name, username=username, lang=lang, referer=referer)
        return user
